...
const selectedBatchWbs = ref<Record<
    string,
...
const selectedApplicant = computed(() => {
    return (
        actionApplicants.value.find(
            (a) => a.value === Number(form.action_applicant_id),
        ) || null
    );
});

const selectedApplicantLabel = computed(() => {
    const selected = actionApplicants.value.find(
        (a) => a.value === Number(form.action_applicant_id),
    );
    return selected ? selected.label : '';
});
...
const initialInterviewActualMin = computed(
    () => form.initial_interview_plan_date || undefined,
);
...
watch(
    ...
    (planDate) => {
        ...
        if (form.initial_interview_actual_date) {
            const actual = parseDateTimeLocal(
...
watch(
    ...
    async (newBatchId) => {
        ...
        try {
            const response = await axios.get(
                `/action/applications/eligible-applicants/${newBatchId}`,
            );

            if (!response.data || response.data.length === 0) {
                noApplicantsError.value =
                    'No eligible applicants found for this batch.';
                actionApplicants.value = [];
                return;
            }
            ...
            noApplicantsError.value =
                ...
                'Failed to load eligible applicants. Please try again.';
            actionApplicants.value = [];
        }
    },
);
...
watch(
    () => form.action_applicant_id,
    async (newApplicantId, oldApplicantId) => {
...
onMounted(() => {
    if (props.actionBatches) {
        actionBatches.value = Object.entries(props.actionBatches).map(
            ([value, label]) => ({
                value: Number(value),
                label: String(label),
            }),
            ...
            <div class="form-wrapper">
                <div class="form-card">
                    <form @submit.prevent="submit">
                        <div class="form-section">
                            ...
                            <div class="form-grid grid-2">
                                ...
                                <div class="form-field">
                                    ...
                                    <span
                                        v-if="noApplicantsError"
                                        class="error-message"
                                        >{{ noApplicantsError }}</span
                                    >
                                    <span
                                        v-if="form.errors.action_applicant_id"
                                        class="error-message"
                                        >{{
                                            form.errors.action_applicant_id
                                        }}</span
                                    >
                                </div>
                            </div>
                        ...
                        <div class="field-label pb-6 text-gray-500">
                            (You may leave the following fields empty if this
                            applicant has not yet started the recruitment
                            process.)
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid grid-3 pt-5">
                                <div class="form-field">
                                    ...
                                    <span
                                        v-if="liveErrors.exam_atpp_result"
                                        class="error-message"
                                    >
                                ...
                                <div class="form-field">
                                    ...
                                    >
                                    <span
                                        v-if="liveErrors.exam_git_result"
                                        class="error-message"
                                ...
                                <div class="form-field">
                                    ...
                                    >
                                    <span
                                        v-if="liveErrors.exam_prg_result"
                                        class="error-message"
                            ...
                            <div class="exam-section-layout">
                                <div class="exam-form-column">
                                    <div
                                        ...
                                        class="criteria-empty"
                                    >
                                        No final interviewers assigned yet. Add
                                        them first in the
