...
const props = defineProps<{
  ...
  prevBatches: { id: number; action_batch: string; action_batch_id?: number }[];
...
onMounted(() => {
  ganttActivities.forEach(act => {
    ganttForm.value[act].start = (props.schedule as any)[`${act}_startdate`] || "";
    ganttForm.value[act].end = act === 'training'
      ? ""
      : ((props.schedule as any)[`${act}_enddate`] || "");
  });
    ...
    <div class="max-w-5xl mx-auto w-full space-y-10 p-8">
      ...
      <div v-if="showError" class="full-width-alert">
        <div class="alert-banner alert-error-banner">
          ...
          <button type="button" class="close-btn" @click="closeError">×</button>
        </div>
      </div>
